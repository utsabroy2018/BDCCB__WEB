...
function DepositForm_BDCCB({ flag }) {
	...
	const handleSubmitSearchAccount = async (value) => {
		console.log('res?.data?.data', 'ressssssssssssssssssssssssss');
		setLoading(true)
		const creds = {
	...
	const fetchGroupDetails = async (value) => {
...
//     }
		...
		const creds = {
			...
			// dep_with_flag : depositWithdrawStatus,
			balance : loanAppData?.flag == "M" ? total_cr_amt : groupDetails[0]?.grp_balance,
