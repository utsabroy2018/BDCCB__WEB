						...
						{(loanAppData && loanAppData.length > 0 ) ? 
							loanAppData?.slice(first, rows + first).map((item, i) => (
								...
								>
									...
									<td className="px-6 py-3 font-bold text-slate-800">{item.sahayika_name || "-----"}</td>
									<td className="px-6 py-3 font-bold text-slate-800">{item.dist_name || "-----"}</td>
									<td className="px-6 py-3 text-slate-700">{item.phone_no}</td>
									<td className="px-6 py-3 text-slate-700">{item.address}</td>
										...
										{/* {flag !== "BM" ? (
											...
											<button
												onClick={() => {
													console.log("LLSKSIODFUISFH", item)
													navigate(
														`/homebm/editSahayikaFormBM/${item?.sahayika_id}`,
														{
															state: item,
														}
													)
...
export default SahayikaTableViewBr
